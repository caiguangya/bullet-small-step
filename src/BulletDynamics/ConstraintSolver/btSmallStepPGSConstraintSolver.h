...
protected:
	...
	btAlignedObjectArray<btSolverBody2> m_tmpSolverBody2Pool;
	...
	void updateConstraints(btScalar elapsedTime, btCollisionObject * *bodies, int numBodies, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer);
	void updateJoint(btScalar elapsedTime, btSolverConstraint * currentConstraintRow, btTypedConstraint * constraint, const btTypedConstraint::btConstraintInfo1& info1, int solverBodyIdA, int solverBodyIdB, const btContactSolverInfo& infoGlobal);
	void updateContact(btScalar elapsedTime, btSolverConstraint & solverConstraint, const btContactSolverInfo& infoGlobal);

	void updateFrictionConstraint(btScalar elapsedTime, btSolverConstraint & solverConstraint, const btQuaternion& deltaRotationB,
	...
	void integrateBodies(int iBegin, int iEnd, const btContactSolverInfo& infoGlobal);
...
public:
	...
	btSmallStepPGSConstraintSolver() : btSequentialImpulseConstraintSolver(), m_subTimeStep(0) {}
